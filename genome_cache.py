...
class GenomeCache:
    ...
    def __init__(self, fasta_path, cache_path):
        ...
        # Load the entire genome directly into RAM (system has 752GB of RAM).
        # This completely eliminates disk page faults and I/O bottlenecks!
        print(f"Loading genome cache into RAM from {self.cache_path}...")
        with open(self.cache_path, 'rb') as f:
            self.data = np.fromfile(f, dtype='uint8', count=self.total_len)
        print("Genome cache loaded into RAM successfully.")
    ...
    def _build_cache(self):
        ...
        import re
        # Match chr1-22, chrX, chrY, chrM (case insensitive)
        pattern = re.compile(r'^chr([1-9]|1[0-9]|2[0-2]|[XYM])$', re.IGNORECASE)
        chroms = sorted([c for c in fasta.keys() if pattern.match(c)])
